...
source_state='local/onefoot-reward-review/pilot-fixedlr/state.json'
python3 - <<'PY'
...
p.write_text(json.dumps({'status':'starting','coordinator_pid':int(os.environ['BRIDGE_COURSE_PID']),
    'started_at':datetime.now(timezone.utc).isoformat(),'plan':['near','unload','transfer-near','transfer','full'],
    'max_updates_per_stage':1000,'num_envs':4096,'stage_runs':[],
    'main_state_promotion':False,'intermediate_goal_s':.5,'final_goal_s':2.},indent=2)+'\n')
PY
BRIDGE_COURSE_OWNED=1
for stage in near unload transfer-near transfer full; do
  if [ "$stage" = near ]; then
    smoke="$base/near-smoke/state.json"
  else
    smoke="$base/${stage}-smoke/state.json"
    uv run --locked python local/train-credit-bridge.py --resume-state "$source_state" \
      ...
      --output "$base/${stage}-smoke" > "$base/${stage}-smoke.log" 2>&1 || { tail -80 "$base/${stage}-smoke.log"; exit 1; }
    tail -1 "$base/${stage}-smoke.log"
  fi
  run="$base/${stage}-training"
...
PY
  ...
  uv run --locked python local/train-credit-bridge.py --resume-state "$source_state" \
    --stage "$stage" --num-envs 4096 --updates 1000 --chunk 100 --eval-episodes 64 \
