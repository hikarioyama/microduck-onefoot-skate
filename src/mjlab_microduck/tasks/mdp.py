...
# --------------------------------------------------------------------------- #
# One-foot roller glide: accelerate, lift RIGHT, coast on LEFT without switching.
# The 3D twist slot is [target_forward_speed, lift_blend, heading_correction].
# This is a NEW command contract, not a drop-in standard velocity policy.
# --------------------------------------------------------------------------- #
class OneFootGlideCommand(CommandTerm):
    def __init__(self, cfg, env):
        super().__init__(cfg, env)
        self.elapsed = torch.zeros(self.num_envs, device=self.device)
        self._command = torch.zeros(self.num_envs, 3, device=self.device)
        self.start_y = torch.zeros_like(self.elapsed)
        self.pending = torch.ones(self.num_envs, dtype=torch.bool, device=self.device)
        self.dwell = torch.zeros_like(self.elapsed)
        self.best_dwell = torch.zeros_like(self.elapsed)
        self.values = {}
        self.cache_step = -1
    ...
    def reset(self, env_ids):
        ...
        self.best_dwell[env_ids] = 0
        self._command[env_ids] = 0
        self._command[env_ids, 0] = self.cfg.target_speed
        self.cache_step = -1
        return {}

    def compute(self, dt):
        # Independent of episode_length_buf: PPO randomizes that buffer at startup.
        self.elapsed += torch.where(self.pending, 0.0, dt)
        self._update_command()

    def _update_command(self):
        robot = self._env.scene['robot']
        y = robot.data.root_link_pos_w[:, 1]
        self.start_y = torch.where(self.pending, y, self.start_y)
        self.pending[:] = False
        self._command[:, 0] = self.cfg.target_speed
        self._command[:, 1] = ((self.elapsed-self.cfg.acceleration_s)/self.cfg.lift_s).clamp(0,1)
        # Lateral line error is observable through the steering correction.
        self._command[:, 2] = (-robot.data.heading_w - 2.0*(y-self.start_y)).clamp(-1,1)
...
@_dataclass(kw_only=True)
class OneFootGlideCommandCfg(CommandTermCfg):
    target_speed: float = 0.3
    acceleration_s: float = 2.5
    lift_s: float = 0.75

    def build(self, env):
        return OneFootGlideCommand(self, env)


def onefoot_scores_from_values(speed, lateral, yaw, cross_track, upright,
                               left_contact, right_contact, clearance,
                               blend, dwell, balance_error, quiet, forbidden):
    """Pure bounded scores; positive form rewards require REAL forward motion.

    No reward for wheel spin alone, flight, right-foot support, body-ground
    contact, or backwards motion. Dwell is continuous valid LEFT support.
    """
    finite = torch.isfinite(torch.stack([speed,lateral,yaw,cross_track,upright,
                                        clearance,blend,dwell,balance_error,quiet],-1)).all(-1)
    posture = ((upright-.65)/.30).clamp(0,1)
    moving = (speed/.25).clamp(0,1)
    straight = torch.exp(-(yaw/.35)**2-(lateral/.15)**2-(cross_track/.20)**2)
    safe = finite & ~forbidden & (left_contact | right_contact)
    common = posture * straight * moving * safe.float()
    left_only = left_contact & ~right_contact & (clearance >= .010)
    # Ramp height rather than rewarding an instantaneous violent leg lift.
    lift = torch.exp(-((clearance-.025*blend)/.020)**2)
    balance = torch.exp(-(balance_error/.040)**2)
    results = {
        'accelerate': common * (1-blend),
        'shape': common * blend * lift * balance,
        'glide': common * blend * left_only.float() * (.25+.75*(dwell/1.5).clamp(0,1)),
        'quiet_glide': common * blend * left_only.float() * quiet,
    }
    return {k:torch.nan_to_num(v,nan=0.0,posinf=0.0,neginf=0.0) for k,v in results.items()}


def onefoot_bad_contact(env):
    return env.scene['onefoot_body_ground'].data.found.flatten(1).any(1)


def _onefoot_values(env):
    cmd = env.command_manager.get_term('twist')
    if cmd.cache_step == env.common_step_counter:
        return cmd.values
    robot = env.scene['robot']
    if not hasattr(cmd, 'wheel_ids'):
        cmd.wheel_ids = [robot.find_bodies(n)[0][0] for n in ('tire','tire_2','tire_3','tire_4')]
        cmd.leg_ids = robot.find_joints(r'^(left|right)_(hip_.*|knee|ankle)$')[0]
    wheels = robot.data.body_link_pos_w[:,cmd.wheel_ids]
    # Both support wheels must touch; ANY swing wheel contact invalidates glide.
    left = env.scene['onefoot_left'].data.found.flatten(1).all(1)
    right = env.scene['onefoot_right'].data.found.flatten(1).any(1)
    forbidden = onefoot_bad_contact(env)
    clearance = wheels[:,2:,2].amin(1)-.0175
    vel = robot.data.root_link_lin_vel_w
    yaw = robot.data.heading_w
    upright = -robot.data.projected_gravity_b[:,2]
    cross_track = robot.data.root_link_pos_w[:,1]-cmd.start_y
    blend = cmd.command[:,1]
    # Genuine sustained straight glide, not just an airborne leg timer.
    valid = (left & ~right & ~forbidden & (clearance >= .010) &
             (vel[:,0] > .10) & (upright > .80) & (yaw.abs() < .35) &
             (cross_track.abs() < .20) & (vel[:,1].abs() < .15) & (blend >= .99))
    cmd.dwell = torch.where(valid, cmd.dwell+env.step_dt, 0.0)
    cmd.best_dwell = torch.maximum(cmd.best_dwell,cmd.dwell)
    # Use whole-robot subtree CoM, NOT root_com_pos_w (trunk-only CoM).
    com = robot.data.data.subtree_com[:,robot.data.indexing.root_body_id]
    balance_error = com[:,1]-wheels[:,:2,1].mean(1)
    quiet = torch.exp(-robot.data.joint_vel[:,cmd.leg_ids].square().mean(1)/16.0)
    vals = onefoot_scores_from_values(vel[:,0],vel[:,1],yaw,cross_track,upright,
                                     left,right,clearance,blend,cmd.dwell,
                                     balance_error,quiet,forbidden)
    vals.update(speed=vel[:,0], clearance=clearance, cross_track=cross_track.abs(),
                heading_error=yaw.abs(), single_support=valid.float(),
                dwell=cmd.dwell.clone(), best_dwell=cmd.best_dwell.clone(),
                success=(cmd.best_dwell >= 2.0).float())
    cmd.values = vals
    cmd.cache_step = env.common_step_counter
    return vals


def onefoot_reward(env, component: str):
    return _onefoot_values(env)[component]


def onefoot_metric(env, component: str):
    return _onefoot_values(env)[component]

# Dynamic one-foot v2. Keep v1 callable semantics intact for archived checkpoints.
class DynamicOneFootCommand(OneFootGlideCommand):
    def __init__(self, cfg, env):
        super().__init__(cfg, env)
        self.previous_com_y = torch.zeros_like(self.elapsed)
        self.previous_support_y = torch.zeros_like(self.elapsed)
        self.dynamic_cache_step = -1
        self.dynamic_values = {}
        robot = env.scene['robot']
        self.support_body_ids = [robot.find_bodies(n)[0][0] for n in ('tire','tire_2')]
    ...
    def _update_command(self):
        reset = self.pending.clone()
        seeded = getattr(self._env, '_onefoot_seeded', torch.zeros_like(reset))
        self.elapsed = torch.where(reset & seeded,
            self.cfg.acceleration_s+self.cfg.lift_s, self.elapsed)
        super()._update_command()
        robot = self._env.scene['robot']
        com_y = robot.data.data.subtree_com[:,robot.data.indexing.root_body_id,1]
        support_y = robot.data.body_link_pos_w[:,self.support_body_ids,1].mean(1)
        # Initial references are taken AFTER reset forward(), not from stale data.
        self.previous_com_y = torch.where(reset,com_y,self.previous_com_y)
        self.previous_support_y = torch.where(reset,support_y,self.previous_support_y)


@_dataclass(kw_only=True)
class DynamicOneFootCommandCfg(OneFootGlideCommandCfg):
    def build(self, env):
        return DynamicOneFootCommand(self, env)


def onefoot_capture_error(com_y, support_y, com_vy, support_vy, height):
    """Lateral capture-point approximation relative to the moving support skate.

    This is shaping, NOT a proof of stability of the articulated rolling robot.
    COM moving toward support is better than the same COM moving away from it.
    """
    return com_y-support_y+(com_vy-support_vy)*torch.sqrt(height.clamp(.05,.30)/9.81)


def dynamic_onefoot_scores(speed, lateral, yaw, cross_track, upright,
                           left_contact, right_contact, clearance, blend,
                           dwell, capture_error, joint_speed_sq, forbidden):
    finite = torch.isfinite(torch.stack([speed,lateral,yaw,cross_track,upright,
        clearance,blend,dwell,capture_error,joint_speed_sq],-1)).all(-1)
    safe = finite & ~forbidden & (left_contact | right_contact)
    transition = (blend > 0) & (blend < 1)
    # Allow lateral movement and lean to transfer weight, tighten AFTER the lift.
    lateral_std = torch.where(transition,.30,.15)
    posture = ((upright-.60)/.35).clamp(0,1)
    straight = torch.exp(-(yaw/.4)**2-(lateral/lateral_std)**2-(cross_track/.25)**2)
    motion = posture*straight*(speed/.25).clamp(0,1)*safe.float()
    left_only = left_contact & ~right_contact & (clearance >= .010)
    capture = torch.exp(-(capture_error/.035)**2)
    # A small partial credit lets the head/leg transfer be discovered; it is
    # bounded to the transition window and never rewards head oscillation itself.
    lift = (clearance.clamp(min=0)/.030).clamp(0,1)
    scores = {
        'progress': motion,
        'transfer': motion*transition.float()*capture*(.20+.80*lift),
        'capture_hold': motion*(blend >= 1).float()*left_only.float()*capture,
        'glide': motion*(blend >= 1).float()*left_only.float()*(.25+.75*(dwell/1.5).clamp(0,1)),
        'settle': motion*(blend >= 1).float()*left_only.float()*torch.exp(-joint_speed_sq/16),
    }
    return {k:torch.nan_to_num(v,nan=0,posinf=0,neginf=0) for k,v in scores.items()}


def _dynamic_onefoot_values(env):
    cmd = env.command_manager.get_term('twist')
    if cmd.dynamic_cache_step == env.common_step_counter:
        return cmd.dynamic_values
    base = _onefoot_values(env)
    robot = env.scene['robot']
    com = robot.data.data.subtree_com[:,robot.data.indexing.root_body_id]
    wheels = robot.data.body_link_pos_w[:,cmd.wheel_ids]
    support_y = wheels[:,:2,1].mean(1)
    com_vy = (com[:,1]-cmd.previous_com_y)/env.step_dt
    support_vy = (support_y-cmd.previous_support_y)/env.step_dt
    height = com[:,2]-wheels[:,:2,2].mean(1)
    capture_error = onefoot_capture_error(com[:,1],support_y,com_vy,support_vy,height)
    cmd.previous_com_y = com[:,1].clone()
    cmd.previous_support_y = support_y.clone()
    if not hasattr(cmd,'dynamic_joint_ids'):
        cmd.dynamic_joint_ids = robot.find_joints(r'^(?!passive_).*')[0]
        cmd.head_joint_ids = robot.find_joints(r'^(neck_pitch|head_pitch|head_yaw|head_roll)$')[0]
    joint_speed_sq = robot.data.joint_vel[:,cmd.dynamic_joint_ids].square().mean(1)
    head_speed = robot.data.joint_vel[:,cmd.head_joint_ids].square().mean(1).sqrt()
    left = env.scene['onefoot_left'].data.found.flatten(1).all(1)
    right = env.scene['onefoot_right'].data.found.flatten(1).any(1)
    vel = robot.data.root_link_lin_vel_w
    blend = cmd.command[:,1]
    vals = dynamic_onefoot_scores(vel[:,0],vel[:,1],robot.data.heading_w,
        robot.data.root_link_pos_w[:,1]-cmd.start_y,-robot.data.projected_gravity_b[:,2],
        left,right,base['clearance'],blend,cmd.dwell,capture_error,joint_speed_sq,
        onefoot_bad_contact(env))
    # Capture movement is a metric, never a standalone swing-speed reward.
    vals.update({k:base[k] for k in ('speed','clearance','cross_track','heading_error',
                                   'single_support','dwell','best_dwell','success')})
    wheel_line = wheels[:,0,:2]-wheels[:,1,:2]
    support_yaw = torch.atan2(wheel_line[:,1],wheel_line[:,0])
    vals.update(onefoot_steering_scores(support_yaw,capture_error,blend,
        base['clearance'],left,right,vals['progress']))
    vals.update(capture_error=capture_error.abs(),com_offset=(com[:,1]-support_y).abs(),
                com_lateral_speed=com_vy,head_speed=head_speed,support_yaw=support_yaw,
                transition=((blend>0)&(blend<1)).float())
    seeded = getattr(env, '_onefoot_seeded', torch.zeros(env.num_envs,device=env.device,dtype=torch.bool))
    vals.update(seeded_start=seeded.float(),launch_fraction=(~seeded).float(),
                launch_success=vals['success']*(~seeded).float(),
                launch_best_dwell=vals['best_dwell']*(~seeded).float())
    cmd.dynamic_values = vals
    cmd.dynamic_cache_step = env.common_step_counter
    return vals


def dynamic_onefoot_reward(env, component: str):
    return _dynamic_onefoot_values(env)[component]


def dynamic_onefoot_metric(env, component: str):
    return _dynamic_onefoot_values(env)[component]


def dynamic_onefoot_motion_cost(env, kind: str):
    """Relax motion blockers in the transition, not throughout the full episode."""
    blend = env.command_manager.get_command('twist')[:,1]
    in_transfer = (blend>0)&(blend<1)
    gate = torch.where(in_transfer,.05,1.0)
    robot = env.scene['robot']
    if kind == 'angular':
        cost = robot.data.root_link_ang_vel_b.square().sum(1)
    elif kind == 'action_rate':
        cost = (env.action_manager.action-env.action_manager.prev_action).square().sum(1)
    else:
        raise ValueError(kind)
    return gate*torch.nan_to_num(cost,nan=0,posinf=0,neginf=0)


def dynamic_onefoot_failure(env):
    """A fall is costly: don't harvest launch rewards then deliberately reset."""
    return env.termination_manager.terminated.float()


def onefoot_steering_scores(support_yaw,capture_error,blend,clearance,left,right,motion):
    """Steer the LEFT blade inward under the predicted COM, then return to +X.

    Actual wheel-axis yaw is measured, never inferred from hip joint sign.
    At HOME positive left_hip_yaw turns the blade right (negative world yaw).
    Capture error is negative when support is left of the COM. A 12cm lookahead
    limits inward steering to 20 degrees; once centered the target is straight.
    No reward for a stationary/airborne steering motion or head shaking itself.
    """
    target=torch.atan2(capture_error,torch.full_like(capture_error,.12)).clamp(-.35,0)
    transfer=((blend>0)&(blend<1)).float()
    lift=(clearance.clamp(min=0)/.025).clamp(0,1)
    error=torch.atan2(torch.sin(support_yaw-target),torch.cos(support_yaw-target))
    inward=motion*left.float()*transfer*(.10+.90*lift)*torch.exp(-(error/.18)**2)
    straight=motion*(blend>=1).float()*(left&~right&(clearance>=.010)).float()*torch.exp(-(support_yaw/.15)**2)
    return {'steer_inward':torch.nan_to_num(inward,nan=0,posinf=0,neginf=0),
            'blade_straight':torch.nan_to_num(straight,nan=0,posinf=0,neginf=0),
            'steering_target':target}


def reset_dynamic_onefoot_seed(env,env_ids,probability:float,pose:dict,root_z:float,root_roll:float):
    """Mix near-goal starts with normal launches. Reset assistance is explicit.

    It is NOT evidence the policy can transfer from two feet. The measured seed
    itself holds only ~0.26s with fixed joint targets; feedback still has to learn.
    Defaults/observations/actions stay HOME-relative, identical between start types.
    """
    if env_ids is None: env_ids=torch.arange(env.num_envs,device=env.device)
    if not hasattr(env,'_onefoot_seeded'):
        env._onefoot_seeded=torch.zeros(env.num_envs,dtype=torch.bool,device=env.device)
    chosen=torch.rand(len(env_ids),device=env.device)<probability
    env._onefoot_seeded[env_ids]=chosen
    selected=env_ids[chosen]
    if len(selected)==0: return
    robot=env.scene['robot']; data=robot.data
    # Reset events precede forward(): read integrated qpos, never stale xpos.
    roots=data.data.qpos[selected[:,None],data.indexing.free_joint_q_adr].clone()
    roots[:,2]=env.scene.env_origins[selected,2]+root_z+.002
    roll=root_roll+torch.empty(len(selected),device=env.device).uniform_(-.01,.01)
    roots[:,3:]=0; roots[:,3]=torch.cos(roll/2); roots[:,4]=torch.sin(roll/2)
    velocity=torch.zeros(len(selected),6,device=env.device)
    velocity[:,0]=torch.empty(len(selected),device=env.device).uniform_(.25,.35)
    # Read default joint arrays and replace only explicitly named joints.
    q=data.default_joint_pos[selected].clone(); dq=torch.zeros_like(q)
    for name,value in pose.items():
        q[:,robot.find_joints(name)[0][0]]=value
    for name in ('passive_LF_wheel','passive_LR_wheel','passive_RF_wheel','passive_RR_wheel'):
        dq[:,robot.find_joints(name)[0][0]]=velocity[:,0]/.0175
    robot.write_root_link_pose_to_sim(roots,env_ids=selected)
    robot.write_root_link_velocity_to_sim(velocity,env_ids=selected)
    robot.write_joint_state_to_sim(q,dq,env_ids=selected)

# v4: alignment must be learnable BEFORE the right skate lifts. Keep v3 intact.
def onefoot_v4_alignment_values(support_yaw,capture_target,blend,left_contact):
    """Smooth inward excursion, then straight, including DOUBLE support and rest.

    Do not gate the cost by forward speed or by already having lifted the foot:
    those gates were the v3 catch-22. Acceleration remains free to use a V stroke.
    """
    blend=blend.clamp(0,1)
    target=capture_target*torch.sin(torch.pi*blend)
    target=torch.where(blend>=1,torch.zeros_like(target),target)
    error=torch.atan2(torch.sin(support_yaw-target),torch.cos(support_yaw-target))
    ramp=((blend-.35)/.65).clamp(0,1)
    ramp=ramp.square()*(3-2*ramp)
    cost=left_contact.float()*ramp*(error/.35).square().clamp(max=16)
    return torch.nan_to_num(cost,nan=0,posinf=0,neginf=0),target


def onefoot_v4_alignment_cost(env):
    v=_dynamic_onefoot_values(env)
    left=env.scene['onefoot_left'].data.found.flatten(1).all(1)
    blend=env.command_manager.get_command('twist')[:,1]
    return onefoot_v4_alignment_values(v['support_yaw'],v['steering_target'],blend,left)[0]


def onefoot_v4_progress(env):
    v=_dynamic_onefoot_values(env)
    left=env.scene['onefoot_left'].data.found.flatten(1).all(1)
    right=env.scene['onefoot_right'].data.found.flatten(1).any(1)
    single=left & ~right & (v['clearance']>=.010)
    blend=env.command_manager.get_command('twist')[:,1]
    # Launch rewards fade unless the robot actually completes the transfer.
    return v['progress']*((1-blend)+blend*single.float())


def onefoot_v4_steer_reward(env):
    v=_dynamic_onefoot_values(env)
    left=env.scene['onefoot_left'].data.found.flatten(1).all(1)
    blend=env.command_manager.get_command('twist')[:,1]
    _,target=onefoot_v4_alignment_values(v['support_yaw'],v['steering_target'],blend,left)
    error=torch.atan2(torch.sin(v['support_yaw']-target),torch.cos(v['support_yaw']-target))
    lift=(v['clearance'].clamp(min=0)/.025).clamp(0,1)
    return v['progress']*left.float()*((blend>0)&(blend<1)).float()*(.1+.9*lift)*torch.exp(-(error/.18)**2)


def onefoot_v4_missed_transfer(env,grace_s:float=.75):
    """Bound unproductive two-foot episodes after a generous transfer window.

    Purely a failed-attempt termination, never a time_out (which PPO bootstraps).
    Failed long standing/rolling therefore cannot avoid the failure cost forever.
    """
    cmd=env.command_manager.get_term('twist')
    left=env.scene['onefoot_left'].data.found.flatten(1).all(1)
    right=env.scene['onefoot_right'].data.found.flatten(1).any(1)
    expired=cmd.elapsed>cmd.cfg.acceleration_s+cmd.cfg.lift_s+grace_s
    return expired & (~left|right) & (cmd.best_dwell<2.0)


def onefoot_v4_failure(env):
    cmd=env.command_manager.get_term('twist')
    # An unsuccessful timeout is not a successful escape from the goal either.
    return (env.termination_manager.terminated |
            (env.termination_manager.time_outs & (cmd.best_dwell<2.0))).float()


# --------------------------------------------------------------------------- #
# v5: gated reverse curriculum. Historical v1/v3/v4 behavior is unchanged.
# Mesh measurements: tire radius is 15 mm, NOT the old assumed 17.5 mm.
# Use a conservative enclosing radius for swing-foot clearance (camber-safe).
# --------------------------------------------------------------------------- #
CURRICULUM_WHEEL_RADIUS = .015
CURRICULUM_WHEEL_BOUND = .016


class CurriculumOneFootCommand(OneFootGlideCommand):
    def __init__(self, cfg, env):
        super().__init__(cfg, env)
        self.good_steps = torch.zeros(self.num_envs, dtype=torch.long, device=self.device)
        self.best_steps = torch.zeros_like(self.good_steps)
        self.missing_steps = torch.zeros_like(self.good_steps)
        self.previous_com = torch.zeros(self.num_envs, 3, device=self.device)
        self.previous_support_y = torch.zeros_like(self.elapsed)
        self.start_x = torch.zeros_like(self.elapsed)
        self.curriculum_values = {}
        self.curriculum_cache_step = -1
        robot = env.scene['robot']
        self.wheel_ids = [robot.find_bodies(n)[0][0] for n in ('tire','tire_2','tire_3','tire_4')]
        self.servo_ids = robot.find_joints(r'^(?!passive_).*')[0]
        self.head_ids = robot.find_joints(r'^(neck_pitch|head_pitch|head_yaw|head_roll)$')[0]
        # Python-list advanced indexing rebuilds and synchronously copies CUDA
        # indices on every use. Resolve by name as before, then retain the tensors.
        self.wheel_ids_tensor = torch.tensor(self.wheel_ids, dtype=torch.long, device=self.device)
        self.head_ids_tensor = torch.tensor(self.head_ids, dtype=torch.long, device=self.device)

    def reset(self, env_ids):
        result = super().reset(env_ids)
        if env_ids is None:
            env_ids = slice(None)
        self.good_steps[env_ids] = 0
        self.best_steps[env_ids] = 0
        self.missing_steps[env_ids] = 0
        self.curriculum_cache_step = -1
        return result

    def _update_command(self):
        pending = self.pending.clone()
        offset = getattr(self._env, '_curriculum_phase_start', torch.zeros_like(self.elapsed))
        self.elapsed = torch.where(pending, offset, self.elapsed)
        super()._update_command()
        robot = self._env.scene['robot']
        com = robot.data.data.subtree_com[:, robot.data.indexing.root_body_id]
        sy = robot.data.body_link_pos_w[:, self.wheel_ids_tensor[:2], 1].mean(1)
        # References must be taken after reset forward(), never from reset events.
        self.previous_com = torch.where(pending[:,None], com, self.previous_com)
        self.previous_support_y = torch.where(pending, sy, self.previous_support_y)
        self.start_x = torch.where(pending, robot.data.root_link_pos_w[:,0], self.start_x)


@_dataclass(kw_only=True)
class CurriculumOneFootCommandCfg(OneFootGlideCommandCfg):
    goal_s: float = .5
    transfer_grace_s: float = .6

    def build(self, env):
        return CurriculumOneFootCommand(self, env)


def reset_curriculum_onefoot(env, env_ids, pose: dict, root_z: float, root_roll: float,
                             speed_range: tuple, phase_start: float,
                             roll_noise: float = .008, joint_noise: float = .004):
    """Spawn-only assistance; never change velocity or apply forces during a run."""
    if env_ids is None:
        env_ids = torch.arange(env.num_envs, device=env.device)
    ...
    roots[:,3:] = 0
    roots[:,3] = torch.cos(roll/2); roots[:,4] = torch.sin(roll/2)
    ...
    for name in ('passive_LF_wheel','passive_LR_wheel','passive_RF_wheel','passive_RR_wheel'):
        dq[:,robot.find_joints(name)[0][0]] = velocity[:,0]/CURRICULUM_WHEEL_RADIUS
...
def curriculum_onefoot_valid(speed, lateral, heading, cross_track, upright,
                            blade_yaw, left, right, clearance, phase, forbidden):
    """Strict common success definition, unchanged as the requested duration grows."""
    finite = torch.isfinite(torch.stack((speed,lateral,heading,cross_track,upright,
                                        blade_yaw,clearance,phase),-1)).all(-1)
    return (finite & left & ~right & ~forbidden & (clearance >= .010) &
            (speed > .10) & (speed < .8) & (lateral.abs() < .15) &
            (heading.abs() < .35) & (cross_track.abs() < .20) &
            (upright > .80) & (blade_yaw.abs() < .35) & (phase >= .99))


def _curriculum_onefoot_values(env):
    cmd = env.command_manager.get_term('twist')
    if cmd.curriculum_cache_step == env.common_step_counter:
        return cmd.curriculum_values
    robot = env.scene['robot']; data = robot.data
    wheels = data.body_link_pos_w[:,getattr(cmd,'wheel_ids_tensor',cmd.wheel_ids)]
    com = data.data.subtree_com[:,data.indexing.root_body_id]
    com_v = (com-cmd.previous_com)/env.step_dt
    support_y = wheels[:,:2,1].mean(1)
    support_vy = (support_y-cmd.previous_support_y)/env.step_dt
    capture = onefoot_capture_error(com[:,1],support_y,com_v[:,1],support_vy,
                                   com[:,2]-wheels[:,:2,2].mean(1))
    cmd.previous_com = com.clone(); cmd.previous_support_y = support_y.clone()
    lf = env.scene['onefoot_left'].data
    rf = env.scene['onefoot_right'].data
    left_force = lf.force[...,2].reshape(env.num_envs,-1).abs()
    right_force = rf.force[...,2].reshape(env.num_envs,-1).abs()
    # A mere collision-detector hit is not evidence of loaded wheel support.
    left = lf.found.reshape(env.num_envs,-1).all(1) & (left_force > .02).all(1)
    left_any = lf.found.reshape(env.num_envs,-1).any(1) & (left_force.sum(1) > .1)
    right = rf.found.reshape(env.num_envs,-1).any(1)
    clearance = wheels[:,2:,2].amin(1)-CURRICULUM_WHEEL_BOUND
    line = wheels[:,0,:2]-wheels[:,1,:2]
    blade = torch.atan2(line[:,1],line[:,0])
    heading = data.heading_w
    cross = data.root_link_pos_w[:,1]-cmd.start_y
    upright = -data.projected_gravity_b[:,2]
    phase = cmd.command[:,1]
    forbidden = onefoot_bad_contact(env)
    # Root velocity retains the original success contract; COM speed is logged
    # separately to detect a torso-motion/centroid discrepancy rather than hiding it.
    velocity = data.root_link_lin_vel_w
    valid = curriculum_onefoot_valid(velocity[:,0],velocity[:,1],heading,cross,upright,
                                     blade,left,right,clearance,phase,forbidden)
    cmd.good_steps = torch.where(valid,cmd.good_steps+1,0)
    cmd.best_steps = torch.maximum(cmd.best_steps,cmd.good_steps)
    cmd.dwell = cmd.good_steps*env.step_dt; cmd.best_dwell = cmd.best_steps*env.step_dt
    cmd.missing_steps = torch.where(left & ~right,0,cmd.missing_steps+1)
    support = left & ~right & (clearance >= .010) & ~forbidden
    finite = torch.isfinite(torch.stack((velocity[:,0],velocity[:,1],capture,upright,blade),-1)).all(-1)
    safe = finite & ~forbidden & (left_any | right)
    posture = ((upright-.65)/.30).clamp(0,1)
    moving = (velocity[:,0]/.25).clamp(0,1)
    straight = torch.exp(-(heading/.45).square()-(cross/.25).square()-(velocity[:,1]/.22).square())
    common = safe.float()*posture*moving*straight
    transfer = (phase>0)&(phase<1)
    capture_score = torch.exp(-(capture/.030).square())
    lift_score = torch.exp(-((clearance-.025*phase)/.025).square())
    # No late double-support survival rewards. The only tail reward requires
    # loaded LEFT-only support; modest steering corrections remain available.
    tail = common*support.float()*(phase>=.99).float()
    track_target = torch.atan2(capture,torch.full_like(capture,.12)).clamp(-.35,.35)
    track_error = torch.atan2(torch.sin(blade-track_target),torch.cos(blade-track_target))
    total_force = left_force.sum(1)+right_force.sum(1)
    left_load = left_force.sum(1)/total_force.clamp_min(.01)
    values = dict(
        hold=tail*(.35+.65*capture_score),
        continuous=valid.float(),
        transfer=common*transfer.float()*left_any.float()*capture_score*(.15+.85*lift_score)*left_load,
        accelerate=common*(phase<=0).float(),
        # Bound alignment cost, with a deadband: balance needs small steering.
        alignment=safe.float()*phase*((blade.abs()-.15).clamp_min(0)/.35).square().clamp_max(4),
        steering=common*transfer.float()*left_any.float()*torch.exp(-(track_error/.25).square()),
        speed=velocity[:,0],com_speed=com_v[:,0],lateral=velocity[:,1].abs(),
        clearance=clearance,upright=upright,cross_track=cross.abs(),heading_error=heading.abs(),
        support_yaw=blade.abs(),capture_error=capture.abs(),com_offset=(com[:,1]-support_y).abs(),
        single_support=valid.float(),left_contact=left.float(),right_contact=right.float(),
        left_load=left_load,left_force=left_force.sum(1),right_force=right_force.sum(1),
        dwell=cmd.dwell.clone(),best_dwell=cmd.best_dwell.clone(),
        success=(cmd.best_steps>=round(cmd.cfg.goal_s/env.step_dt)).float(),
        success_050=(cmd.best_steps>=round(.5/env.step_dt)).float(),
        success_100=(cmd.best_steps>=round(1./env.step_dt)).float(),
        success_200=(cmd.best_steps>=round(2./env.step_dt)).float(),
        distance=data.root_link_pos_w[:,0]-cmd.start_x,
        injected_speed=env._curriculum_injected_speed.clone(),
        head_speed=data.joint_vel[:,getattr(cmd,'head_ids_tensor',cmd.head_ids)].square().mean(1).sqrt(),
        phase=phase.clone(),
    )
    cmd.curriculum_values = {k:torch.nan_to_num(v,nan=0.,posinf=0.,neginf=0.) for k,v in values.items()}
    cmd.curriculum_cache_step = env.common_step_counter
    return cmd.curriculum_values


def curriculum_onefoot_signal(env, component: str):
    return _curriculum_onefoot_values(env)[component]


def curriculum_onefoot_stalled(env):
    cmd = env.command_manager.get_term('twist')
    _curriculum_onefoot_values(env)
    expired = cmd.elapsed > cmd.cfg.acceleration_s+cmd.cfg.lift_s+cmd.cfg.transfer_grace_s
    return expired & (cmd.missing_steps*env.step_dt > .12)


def curriculum_onefoot_failure(env):
    # Unsuccessful timeouts are visible to the evaluator, but are not taxed as
    # falls: the initial 0.5s horizon must not punish a 0.48s near-success by -8.
    return env.termination_manager.terminated.float()


# v6: do not trade a continuous glide for repeated swing-foot catch steps.
def continuous_onefoot_factor(dwell, goal_s: float, com_speed):
    """Smooth streak preference, reset by any invalid frame; no terminal jackpot."""
    streak = .10+.90*(dwell/goal_s).clamp(0,1)
    real_motion = (com_speed/.15).clamp(0,1)
    return streak*real_motion


def continuous_onefoot_reward(env, component: str):
    v = _curriculum_onefoot_values(env)
    cmd = env.command_manager.get_term('twist')
    if component in ('hold','continuous'):
        return v[component]*continuous_onefoot_factor(v['dwell'],cmd.cfg.goal_s,v['com_speed'])
    if component in ('accelerate','transfer','steering'):
        # A rocking torso alone is not propulsion of the whole robot.
        return v[component]*(v['com_speed']/.15).clamp(0,1)
    return v[component]


def continuous_onefoot_replanted(env, established_s: float = .10):
    """Once the commanded glide is established, the swing skate may not land.

    Preparation/pushing in double support remains allowed. This ends only an
    already established single-support attempt, never bootstrapping as timeout.
    best_steps is reset per environment and counts only valid post-transfer frames.
    """
    _curriculum_onefoot_values(env)
    cmd = env.command_manager.get_term('twist')
    right = env.scene['onefoot_right'].data.found.flatten(1).any(1)
    established = cmd.best_steps >= round(established_s/env.step_dt)
    return established & right


# v7: the support skate is narrow laterally but also finite fore/aft (65 mm).
def foreaft_onefoot_factor(capture_x, margin: float = .018, width: float = .020):
    excess=(capture_x.abs()-margin).clamp_min(0)
    return torch.nan_to_num(torch.exp(-(excess/width).square()),nan=0.)


def _centroid_onefoot_values(env):
    cmd=env.command_manager.get_term('twist')
    if getattr(cmd,'centroid_cache_step',-1)==env.common_step_counter:
        return cmd.centroid_values
    data=env.scene['robot'].data
    com=data.data.subtree_com[:,data.indexing.root_body_id]
    mass=data.model.body_mass[:,data.indexing.body_ids]
    com_v=(data.body_com_lin_vel_w*mass[...,None]).sum(1)/mass.sum(1,keepdim=True)
    support=data.body_link_pos_w[:,cmd.wheel_ids[:2]].mean(1)
    support_v=data.body_link_lin_vel_w[:,cmd.wheel_ids[:2]].mean(1)
    error=com[:,0]-support[:,0]
    capture=error+(com_v[:,0]-support_v[:,0])*torch.sqrt((com[:,2]-support[:,2]).clamp(.05,.30)/9.81)
    force=env.scene['onefoot_left'].data.force[...,2].reshape(env.num_envs,-1).abs()
    values=dict(foreaft_error=error.abs(),foreaft_capture=capture.abs(),
                front_load=force[:,0]/force.sum(1).clamp_min(.01),factor=foreaft_onefoot_factor(capture))
    cmd.centroid_values=values;cmd.centroid_cache_step=env.common_step_counter
    return values


def centroid_onefoot_reward(env,component: str):
    reward=continuous_onefoot_reward(env,component)
    return reward*_centroid_onefoot_values(env)['factor']


def centroid_onefoot_metric(env,component: str):
    return _centroid_onefoot_values(env)[component]


# v9: pelvis/torso participation, without rewarding tilt magnitude itself.
# Measured nominal whole-body COM relative to the pelvis in the balance spawn.
WAIST_COM_REFERENCE_BODY = (0.002233328475627581, -0.00007587017051351266, 0.021823029594897)


def waist_lean_posture_credit(gravity_b):
    """Allow useful lateral lean; retain pitch preference and fall safety.

    The previous common score saturated only at total tilt <18.2 degrees.
    Here lateral roll is not taxed inside the EXISTING success tilt envelope;
    forward/backward pitching and near-fall attitudes are not made free.
    """
    upright=-gravity_b[:,2]
    pitch_cos=torch.sqrt((1-gravity_b[:,0].square()).clamp(0,1))
    safe=((upright-.65)/.15).clamp(0,1)
    pitch=((pitch_cos-.65)/.30).clamp(0,1)
    return torch.nan_to_num(safe*pitch,nan=0.,posinf=0.,neginf=0.)


def waist_axis_alignment(core_y,support_y,width=.025):
    """A correctly placed upright pelvis is valid too: no nonzero-angle bonus."""
    return torch.nan_to_num(torch.exp(-((core_y-support_y)/width).square()),nan=0.)


def loaded_contact_center(positions,forces_z,found,fallback):
    """Approximate support CoP from each wheel's strongest measured contact."""
    weights=forces_z.abs()*found.to(forces_z.dtype)
    weights=torch.nan_to_num(weights,nan=0.,posinf=0.,neginf=0.)
    total=weights.sum(1,keepdim=True)
    points=torch.nan_to_num(positions,nan=0.,posinf=0.,neginf=0.)
    center=(points*weights[...,None]).sum(1)/total.clamp_min(1e-6)
    return torch.where(total>.02,center,fallback)


def waist_foreaft_support_cost(com_error_x,capture_x,half_wheelbase):
    """Keep COM and relative momentum away from BOTH ends of the support skate.

    This is a soft articulated-body capture approximation, not a ZMP/stability
    proof. A 10 mm reserve is measured inward from the actual wheel-center span.
    """
    inner=(half_wheelbase-.010).clamp_min(.008)
    excess=(capture_x.abs()-inner).clamp_min(0)
    cost=(com_error_x/.040).square()+(excess/.020).square()
    return torch.nan_to_num(cost,nan=0.,posinf=9.,neginf=0.).clamp_max(9.)


def _waist_onefoot_values(env):
    cmd=env.command_manager.get_term('twist')
    if getattr(cmd,'waist_cache_step',-1)==env.common_step_counter:
        return cmd.waist_values
    base=_curriculum_onefoot_values(env)
    robot=env.scene['robot'];data=robot.data
    wheel_ids=getattr(cmd,'wheel_ids_tensor',cmd.wheel_ids)
    wheels=data.body_link_pos_w[:,wheel_ids]
    contact=env.scene['onefoot_left_cop'].data
    support=loaded_contact_center(contact.pos.reshape(env.num_envs,-1,3),
        contact.force[...,2].reshape(env.num_envs,-1),
        contact.found.reshape(env.num_envs,-1)>0,wheels[:,:2].mean(1))
    com=data.data.subtree_com[:,data.indexing.root_body_id]
    mass=data.model.body_mass[:,data.indexing.body_ids]
    com_v=(data.body_com_lin_vel_w*mass[...,None]).sum(1)/mass.sum(1,keepdim=True)
    support_v=data.body_link_lin_vel_w[:,wheel_ids[:2]].mean(1)
    capture=onefoot_capture_error(com[:,1],support[:,1],com_v[:,1],support_v[:,1],com[:,2]-support[:,2])
    center=wheels[:,:2].mean(1)
    foreaft_error=com[:,0]-center[:,0]
    foreaft_capture=onefoot_capture_error(com[:,0],center[:,0],com_v[:,0],support_v[:,0],com[:,2]-center[:,2])
    half_span=(wheels[:,0,0]-wheels[:,1,0]).abs()*.5
    reference=getattr(cmd,'waist_reference_body',None)
    if reference is None:
        reference=data.root_link_pos_w.new_tensor(WAIST_COM_REFERENCE_BODY).expand(env.num_envs,-1)
        cmd.waist_reference_body=reference
    # Fixed neutral COM reference: waving only the head/free leg cannot move this
    # point. Moving or leaning the actual pelvis/torso can. Actual full-body COM
    # and single-support success remain independent checks, so this is only shaping.
    core=data.root_link_pos_w+quat_apply(data.root_link_quat_w,reference)
    axis_score=waist_axis_alignment(core[:,1],support[:,1])
    gravity=data.projected_gravity_b
    roll=torch.atan2(-gravity[:,1],-gravity[:,2])
    pitch=torch.asin(gravity[:,0].clamp(-1,1))
    phase=cmd.command[:,1]
    left=base['left_contact']>.5;right=base['right_contact']>.5
    left_sensor=env.scene['onefoot_left'].data
    left_any=(left_sensor.found.reshape(env.num_envs,-1).any(1) & (base['left_force']>.1))
    safe=~onefoot_bad_contact(env)&(left_any|right)
    force=left_sensor.force[...,2].reshape(env.num_envs,-1).abs()
    front_load=force[:,0]/force.sum(1).clamp_min(.01)
    moving=(base['speed']/.25).clamp(0,1)
    straight=torch.exp(-(base['heading_error']/.45).square()-(base['cross_track']/.25).square()-(base['lateral']/.22).square())
    common=safe.float()*waist_lean_posture_credit(gravity)*moving*straight
    capture_score=torch.exp(-(capture/.030).square())
    lift=torch.exp(-((base['clearance']-.025*phase)/.025).square())
    transfer_phase=(phase>0)&(phase<1)
    single=left&~right&(base['clearance']>=.010)
    tail=common*single.float()*(phase>=.99).float()
    hold=tail*(.35+.65*capture_score)
    transfer=common*transfer_phase.float()*left_any.float()*capture_score*(.15+.85*lift)*base['left_load']
    line=wheels[:,0,:2]-wheels[:,1,:2];yaw=torch.atan2(line[:,1],line[:,0])
    target=torch.atan2(capture,torch.full_like(capture,.12)).clamp(-.35,.35)
    steering_error=torch.atan2(torch.sin(yaw-target),torch.cos(yaw-target))
    streak=continuous_onefoot_factor(base['dwell'],cmd.cfg.goal_s,com_v[:,0])
    real_motion=(com_v[:,0]/.15).clamp(0,1)
    values=dict(hold=hold*streak,continuous=base['single_support']*streak,
        transfer=transfer*real_motion,accelerate=common*(phase<=0).float()*real_motion,
        steering=common*transfer_phase.float()*left_any.float()*torch.exp(-(steering_error/.25).square())*real_motion,
        waist_support=axis_score*(hold*streak+transfer*real_motion),
        foreaft_balance=safe.float()*left_any.float()*phase.square()*waist_foreaft_support_cost(foreaft_error,foreaft_capture,half_span),
        waist_foreaft_error=foreaft_error,waist_foreaft_capture=foreaft_capture,waist_front_load=front_load,
        waist_roll_deg=roll*180/torch.pi,waist_pitch_deg=pitch*180/torch.pi,
        waist_roll_speed=data.root_link_ang_vel_b[:,0].abs(),
        waist_axis_error=(core[:,1]-support[:,1]).abs(),waist_axis_alignment=axis_score,
        waist_pelvis_offset=(data.root_link_pos_w[:,1]-support[:,1]).abs(),
        waist_capture_error=capture.abs(),support_cop_y_offset=support[:,1]-wheels[:,:2,1].mean(1),
        waist_capture_signed=capture,waist_com_height=com[:,2]-support[:,2],waist_roll_radians=roll)
    cmd.waist_values={k:torch.nan_to_num(v,nan=0.,posinf=0.,neginf=0.) for k,v in values.items()}
    cmd.waist_cache_step=env.common_step_counter
    return cmd.waist_values


def waist_onefoot_signal(env,component:str):
    return _waist_onefoot_values(env)[component]


# v10: an explicit, support-side torso reference, not simply permission to lean.
WAIST_REFERENCE_ROLL = -0.27713600810147127


def waist_support_roll_target(phase,capture_error,height):
    """Measured left-support lean plus state-dependent correction/braking.

    Negative roll tilts the trunk toward world +Y (the left support side).
    If the predicted COM escapes right, ask for more left lean. If it has moved
    too far left, ease the lean or counter-lean. No angular-speed reward.
    """
    correction=torch.atan2(capture_error,height.clamp(.05,.30))
    target=(WAIST_REFERENCE_ROLL+correction).clamp(-.45,.20)
    return torch.nan_to_num(target*phase.clamp(0,1),nan=0.)


def waist_target_onefoot_signal(env,component:str):
    values=_waist_onefoot_values(env)
    cmd=env.command_manager.get_term('twist')
    command=cmd.command
    # Three rewards and three metrics request the same target in one control
    # step. Retain exact eager results, invalidated by a new physical-value
    # dictionary, a replaced command buffer, or an in-place command update.
    # Inference tensors have no version counter: conservatively do not cache.
    cacheable=not command.is_inference()
    version=command._version if cacheable else None
    cache=getattr(cmd,'waist_target_cache',None)
    if (not cacheable or cache is None or cache[0] is not values or
            cache[1] is not command or cache[2]!=version):
        target=waist_support_roll_target(command[:,1],values['waist_capture_signed'],values['waist_com_height'])
        error=values['waist_roll_radians']-target
        error=torch.atan2(torch.sin(error),torch.cos(error))
        match=torch.exp(-(error/.20).square())
        if cacheable:cmd.waist_target_cache=(values,command,version,target,error,match)
    else:
        target,error,match=cache[3:]
    if component=='waist_target_roll_deg':return target*180/torch.pi
    if component=='waist_roll_error_deg':return error.abs()*180/torch.pi
    if component=='waist_roll_match':return match
    if component=='waist_support':return values[component]*match
    if component in ('hold','transfer'):
        # Keep partial credit for real corrective motion outside the reference;
        # upright is no longer equally preferred when support-side lean is needed.
        return values[component]*(.35+.65*match)
    return values[component]


# Evaluation-only observer: preserve reward-time state before automatic reset.
def curriculum_onefoot_diagnostic_checks(speed, lateral, heading, cross_track,
        upright, blade_yaw, left, right, clearance, phase, forbidden):
    """Explain the EXISTING valid predicate; never relax or replace its gates."""
    finite=torch.isfinite(torch.stack((speed,lateral,heading,cross_track,upright,
                                      blade_yaw,clearance,phase),-1)).all(-1)
    return torch.stack((finite,left,~right,clearance>=.010,
        (speed>.10)&(speed<.8),lateral.abs()<.15,heading.abs()<.35,
        cross_track.abs()<.20,upright>.80,blade_yaw.abs()<.35,
        phase>=.99,~forbidden),-1)


def observe_curriculum_onefoot_diagnostics(env):
    """Optional evaluation metric, not an observation/reward/termination change.

    The evaluator consumes these copied tensors after env.step(). Reading raw
    body data at that point would instead see reset poses for terminated worlds.
    """
    base=_curriculum_onefoot_values(env)
    cmd=env.command_manager.get_term('twist');data=env.scene['robot'].data
    wheel_ids=getattr(cmd,'wheel_ids_tensor',cmd.wheel_ids)
    wheels=data.body_link_pos_w[:,wheel_ids]
    lf=env.scene['onefoot_left'].data;rf=env.scene['onefoot_right'].data
    left_force=lf.force[...,2].reshape(env.num_envs,-1).abs()
    right_force=rf.force[...,2].reshape(env.num_envs,-1).abs()
    left=lf.found.reshape(env.num_envs,-1).all(1)&(left_force>.02).all(1)
    right=rf.found.reshape(env.num_envs,-1).any(1)
    ...
    line=wheels[:,0,:2]-wheels[:,1,:2]
    blade=torch.atan2(line[:,1],line[:,0])
    velocity=data.root_link_lin_vel_w;gravity=data.projected_gravity_b
    upright=-gravity[:,2];heading=data.heading_w
    cross=data.root_link_pos_w[:,1]-cmd.start_y;phase=cmd.command[:,1]
    checks=curriculum_onefoot_diagnostic_checks(velocity[:,0],velocity[:,1],heading,cross,
        upright,blade,left,right,clearance,phase,onefoot_bad_contact(env))
    values=torch.stack((velocity[:,0],base['com_speed'],clearance,upright,blade,
        base['capture_error'],left_force[:,0],left_force[:,1],right_force.sum(1),
        torch.atan2(-gravity[:,1],-gravity[:,2])*180/torch.pi,
        torch.asin(gravity[:,0].clamp(-1,1))*180/torch.pi),-1)
    cmd.onefoot_diagnostics=dict(step=env.common_step_counter,checks=checks,values=values)
    return checks.all(1).float()


SUPPORT_KINEMATIC_NAMES = (
    'left_knee_rad','left_knee_velocity_rad_s','left_knee_target_rad',
    'left_hip_pitch_rad','left_ankle_rad','pelvis_height_m',
    'pelvis_velocity_z_m_s','com_height_m','support_speed_m_s',
)


def curriculum_onefoot_support_kinematics(env):
    """Read-only physical measurements; knee qpos zero is NOT a straight leg.

    Positive left_knee qpos folds the support leg further (verified on MJCF).
    Keep raw nonfinite values visible to diagnostics rather than hiding them.
    """
    robot=env.scene['robot'];data=robot.data
    cmd=env.command_manager.get_term('twist')
    ids=getattr(cmd,'support_kinematic_joint_ids',None)
    if ids is None:
        ids=tuple(robot.find_joints(name)[0][0] for name in
                  ('left_knee','left_hip_pitch','left_ankle'))
        cmd.support_kinematic_joint_ids=ids
    knee,hip,ankle=ids
    origin_z=env.scene.env_origins[:,2]
    com=data.data.subtree_com[:,data.indexing.root_body_id]
    wheel_ids=getattr(cmd,'wheel_ids_tensor',cmd.wheel_ids)
    return torch.stack((data.joint_pos[:,knee],data.joint_vel[:,knee],
        data.joint_pos_target[:,knee],data.joint_pos[:,hip],data.joint_pos[:,ankle],
        data.root_link_pos_w[:,2]-origin_z,data.root_link_lin_vel_w[:,2],
        com[:,2]-origin_z,data.body_link_lin_vel_w[:,wheel_ids[:2],0].mean(1)),-1)


def observe_curriculum_onefoot_support_kinematics(env):
    """Optional frozen-policy observer; capture joints BEFORE automatic reset."""
    valid=observe_curriculum_onefoot_diagnostics(env)
    cmd=env.command_manager.get_term('twist')
    cmd.onefoot_diagnostics['support_kinematics']=curriculum_onefoot_support_kinematics(env)
    return valid


# v11 bounded candidate: do not retain the deep support-knee collapse strategy.
def support_knee_credit(knee,phase,comfortable_max:float=.35,width:float=.45):
    """Plateau over ordinary flexion; no incentive to lock or straighten the knee.

    These are MJCF joint coordinates, NOT human anatomical angles. At +.35 rad
    this model's leg is still flexed. Only excessive positive qpos reduces the
    existing maneuver credit. No new reward for merely extending or standing.
    Preparation at phase=0 is untouched; transfer blends in smoothly.
    """
    excess=(knee-comfortable_max).clamp_min(0)
    match=torch.exp(-(excess/width).square())
    blend=phase.clamp(0,1).square()
    return torch.nan_to_num(1-blend+blend*match,nan=0.,posinf=0.,neginf=0.)


def knee_guard_onefoot_signal(env,component:str,comfortable_max:float=.35,width:float=.45):
    robot=env.scene['robot'];cmd=env.command_manager.get_term('twist')
    knee_id=getattr(cmd,'support_knee_joint_id',None)
    if knee_id is None:
        knee_id=robot.find_joints('left_knee')[0][0]
        cmd.support_knee_joint_id=knee_id
    knee=robot.data.joint_pos[:,knee_id]
    credit=support_knee_credit(knee,cmd.command[:,1],comfortable_max,width)
    if component=='support_knee_rad':return knee
    if component=='support_knee_credit':return credit
    if component=='support_knee_excess_rad':return (knee-comfortable_max).clamp_min(0)
    reward=waist_target_onefoot_signal(env,component)
    if component in ('hold','continuous','transfer','waist_support'):
        return reward*credit
    return reward


# v15: retained-skill bridge with an auditable finite-episode objective.
# Legacy task rewards above are unchanged. Potential shaping has terminal Phi=0;
# only NEW credible glide records earn task credit. No free launch reward and no
# negative posture-rate bill that can be escaped by ending the episode early.
#
# v16 adds the two levers measured in local/onefoot-bridge-curriculum/
# GLIDE-PHYSICS-DIAGNOSIS.ja.md. A 2.0 s glide needs entry_speed >= .10 + 2*|a|:
# with the observed ~.17 m/s^2 decay that is .44 m/s, but with a balanced ~.13 it
# is only .36 m/s, and today's handoff already reaches ~.336 (the frozen public
# expert plateaus at ~.38 no matter how long the acceleration window is).
# (B) prices a faster handoff; (E) charges the two balance axes so a drifting
# glide discounts its own later credit. Either lever alone can close the gap.
BRIDGE_HANDOFF_REF_SPEED = .38   # frozen public expert's measured speed ceiling
# Raised from .15 after the second v16 block: the median credible glide now starts
# at .546 m/s and decays at .256 m/s^2, so a 2.0 s glide needs .613 m/s at entry.
# With the old span the bonus saturated at .53 and the policy's peak pinned exactly
# there (gradient zero above it), leaving a measured .080 m/s shortfall. The span
# now saturates at .68 m/s, above the requirement, so the gradient survives.
BRIDGE_HANDOFF_SPAN = .30        # full handoff credit at .68 m/s
BRIDGE_HANDOFF_GAIN = 1.2        # below failure_cost, so a reckless lift cannot pay
BRIDGE_HANDOFF_HOLD_S = .25      # the fast lift must survive this much real glide
BRIDGE_LATERAL_DEADBAND = .008   # measured p50 of |lateral capture error|
BRIDGE_LATERAL_WIDTH = .025
BRIDGE_FOREAFT_DEADBAND = .005   # measured p50 of |fore/aft capture error|
BRIDGE_FOREAFT_WIDTH = .020
BRIDGE_BALANCE_RATE = 1.
BRIDGE_FOREAFT_PHI_WIDTH = .02


def credit_bridge_potential(com_speed, skate_speed, capture_error, fore_aft_error,
                            left_front, left_rear, right_force, clearance, upright,
                            blade_yaw, forbidden):
    values = torch.stack((com_speed, skate_speed, capture_error, fore_aft_error,
                          left_front, left_rear, right_force, clearance, upright,
                          blade_yaw), -1)
    finite = torch.isfinite(values).all(-1)
    left = left_front.clamp_min(0)+left_rear.clamp_min(0)
    total = left+right_force.clamp_min(0)
    load = (left/total.clamp_min(.1)).clamp(0, 1)
    pair = (2*torch.minimum(left_front, left_rear)/left.clamp_min(.1)).clamp(0, 1)
    capture = torch.exp(-(capture_error/.05).square())
    # The measured fore/aft capture error drifts from -.015 m (COM behind the skate
    # line) to +.015 m (COM escaping ahead) over a 1 s glide, p90 |error| .0136 m.
    fore_capture = torch.exp(-(fore_aft_error/BRIDGE_FOREAFT_PHI_WIDTH).square())
    lift = (clearance/.010).clamp(0, 1)
    readiness = load*(.45*capture+.25*pair+.30*lift)
    moving = (com_speed/.15).clamp(0, 1)*(skate_speed/.10).clamp(0, 1)
    posture = ((upright-.65)/.30).clamp(0, 1)
    straight = torch.exp(-(blade_yaw/.35).square())
    safe = finite & ~forbidden & (total > .1) & (upright > .65)
    phi = torch.where(safe, readiness*moving*posture*straight*fore_capture, 0.)
    return torch.nan_to_num(phi, nan=0., posinf=0., neginf=0.).clamp(0, 1)


def credit_bridge_balance_cost(lateral_capture, fore_aft_capture, left_only):
    """Excess capture error of the loaded support skate, on BOTH axes.

    Zero inside the measured deadbands, so a normally balanced glide accrues no
    debt at all: over a hypothetical 2.0 s glide this is 0.00 at the measured p50
    balance, 0.21 at p75, 2.03 at p90 and 12.6 at p99. It is charged only while
    the left skate alone carries the robot, so standing on both feet cannot farm
    (or avoid) it. The value is a DEBT that discounts later glide credit and is
    never a negative continuing reward, so ending an episode early cannot escape
    an outstanding balance bill.
    """
    zero = torch.zeros_like(lateral_capture)
    lateral = ((lateral_capture.abs()-BRIDGE_LATERAL_DEADBAND).clamp_min(0)
               /BRIDGE_LATERAL_WIDTH).square()
    fore_aft = ((fore_aft_capture.abs()-BRIDGE_FOREAFT_DEADBAND).clamp_min(0)
                /BRIDGE_FOREAFT_WIDTH).square()
    cost = BRIDGE_BALANCE_RATE*(lateral+fore_aft)
    cost = torch.nan_to_num(cost, nan=1e6, posinf=1e6, neginf=0)
    return torch.where(left_only, cost, zero).clamp_min(0)


class CreditBridgeRewardState:
    """Pure vectorized episode accounting, shared by runtime and CPU replay.

    Returned values are per-control-step rewards, NOT rates. With gamma matched
    to PPO and terminal Phi=0, sum gamma**t * shaping[t] = -weight*Phi_initial.
    Phi_initial is explicitly zero at each reset. Hence failed motion/loitering
    cannot earn shaping profit, regardless of its potential path or duration.
    """
    def __init__(self, n, device, *, dt=.02, gamma=.99, goal_s=1.,
                 glide_rate=12., potential_weight=1., failure_cost=2.,
                 handoff_ref=BRIDGE_HANDOFF_REF_SPEED, handoff_span=BRIDGE_HANDOFF_SPAN,
                 handoff_gain=BRIDGE_HANDOFF_GAIN, handoff_hold_s=BRIDGE_HANDOFF_HOLD_S):
        if not (dt > 0 and 0 < gamma <= 1 and goal_s > 0 and glide_rate > 0
                and potential_weight >= 0 and failure_cost > 0):
            raise ValueError('Invalid finite-episode reward parameters')
        if not (handoff_span > 0 and handoff_gain >= 0 and handoff_hold_s >= 0):
            raise ValueError('Invalid handoff credit parameters')
        if n < 1 or round(goal_s/dt) < 1:
            raise ValueError('Expected nonempty batch and a goal of at least one control step')
        self.dt, self.gamma, self.goal_s = dt, gamma, goal_s
        self.glide_rate, self.potential_weight, self.failure_cost = glide_rate, potential_weight, failure_cost
        self.handoff_ref, self.handoff_span = handoff_ref, handoff_span
        self.handoff_gain = handoff_gain
        self.handoff_steps = max(1, round(handoff_hold_s/dt))
        self.previous_phi = torch.zeros(n, device=device)
        self.smooth_debt = torch.zeros(n, device=device)
        self.balance_debt = torch.zeros(n, device=device)
        self.good_steps = torch.zeros(n, dtype=torch.long, device=device)
        self.best_steps = torch.zeros_like(self.good_steps)
        self.entry_speed = torch.zeros(n, device=device)
        self.established = torch.zeros(n, dtype=torch.bool, device=device)
        self.handoff_paid = torch.zeros(n, dtype=torch.bool, device=device)

    def reset(self, env_ids=None):
        if env_ids is None:
            env_ids = slice(None)
        self.previous_phi[env_ids] = 0.
        self.smooth_debt[env_ids] = 0.
        self.balance_debt[env_ids] = 0.
        self.good_steps[env_ids] = 0
        self.best_steps[env_ids] = 0
        self.entry_speed[env_ids] = 0.
        self.established[env_ids] = False
        self.handoff_paid[env_ids] = False

    def advance(self, *, phi, valid, com_speed, skate_speed, smooth_cost,
                done, physical_failure, balance_cost=None):
        finite = torch.isfinite(com_speed) & torch.isfinite(skate_speed) & torch.isfinite(smooth_cost)
        credible = valid & finite & ~physical_failure & (com_speed > .10) & (skate_speed > .05)
        self.good_steps.copy_(torch.where(credible, self.good_steps+1, 0))
        best = torch.maximum(self.best_steps, self.good_steps)
        progress = (best-self.best_steps).clamp(0, 1)
        self.best_steps.copy_(best)
        safe_cost = torch.nan_to_num(smooth_cost, nan=1e6, posinf=1e6, neginf=0).clamp_min(0)
        balance = (torch.zeros_like(safe_cost) if balance_cost is None else
                   torch.nan_to_num(balance_cost, nan=1e6, posinf=1e6, neginf=0).clamp_min(0))
        self.smooth_debt += (safe_cost+balance)*self.dt
        self.balance_debt += balance*self.dt
        quality = torch.reciprocal(1+self.smooth_debt)
        # (B) One handoff bonus per episode, priced only above the frozen expert's
        # own measured ceiling, and paid only after the fast lift has survived
        # handoff_hold_s of real glide so a throw-and-fall earns nothing. The gain
        # is below failure_cost, so the bonus can never make a fall profitable.
        speed = torch.nan_to_num(com_speed, nan=0., posinf=0., neginf=0.)
        fresh = valid & ~self.established
        self.entry_speed.copy_(torch.where(fresh, speed, self.entry_speed))
        self.established |= valid
        handoff_factor = ((self.entry_speed-self.handoff_ref)/self.handoff_span).clamp(0, 1)
        due = (self.good_steps >= self.handoff_steps) & ~self.handoff_paid
        handoff = torch.where(due, self.handoff_gain*handoff_factor,
                              torch.zeros_like(handoff_factor))
        self.handoff_paid |= due
        # Remember transfer-time overdrive when discounting LATER glide credit.
        # Waiting cannot erase it. This never becomes a negative continuing
        # reward, so terminating early cannot escape an outstanding penalty bill.
        task = progress.float()*self.dt*self.glide_rate*quality
        current_phi = torch.nan_to_num(phi, nan=0., posinf=0., neginf=0.).clamp(0, 1)
        terminal_phi = torch.where(done, 0., current_phi)
        shaping = self.potential_weight*(self.gamma*terminal_phi-self.previous_phi)
        self.previous_phi.copy_(terminal_phi)
        goal_steps = round(self.goal_s/self.dt)
        missed_goal = done & (best < goal_steps)
        failed = physical_failure | missed_goal
        failure = -self.failure_cost*failed.float()
        return dict(task=task, shaping=shaping, failure=failure, handoff=handoff,
                    total=task+shaping+failure+handoff, credible=credible.float(),
                    credible_best_s=best.float()*self.dt, progress_steps=progress.float(),
                    potential=terminal_phi.clone(), quality=quality,
                    failed=failed.float(), missed_goal=missed_goal.float(),
                    entry_speed=self.entry_speed.clone(), handoff_factor=handoff_factor,
                    balance_debt=self.balance_debt.clone(), balance_cost=balance)


class CreditBridgeCommand(CurriculumOneFootCommand):
    def __init__(self, cfg, env):
        super().__init__(cfg, env)
        self.bridge_reward_state = CreditBridgeRewardState(self.num_envs, self.device,
            dt=env.step_dt, gamma=cfg.reward_gamma, goal_s=cfg.goal_s,
            glide_rate=cfg.glide_rate, potential_weight=cfg.potential_weight, failure_cost=cfg.failure_cost)
        self.bridge_reward_cache_step = -1
        self.bridge_reward_values = {}
        # Fore/aft capture is reconstructed here (not in the frozen legacy region)
        # with the same one-step difference the lateral axis uses. The mask keeps
        # the first step after any partial reset from differencing across it.
        self._bridge_previous_com = None
        self._bridge_previous_support = None
        self._bridge_previous_valid = None

    def reset(self, env_ids):
        result = super().reset(env_ids)
        if hasattr(self, 'bridge_reward_state'):
            self.bridge_reward_state.reset(env_ids)
        if getattr(self, '_bridge_previous_valid', None) is not None:
            self._bridge_previous_valid[slice(None) if env_ids is None else env_ids] = False
        self.bridge_reward_cache_step = -1
        return result


@_dataclass(kw_only=True)
class CreditBridgeCommandCfg(CurriculumOneFootCommandCfg):
    reward_gamma: float = .99
    glide_rate: float = 12.
    potential_weight: float = 1.
    failure_cost: float = 2.

    def build(self, env):
        return CreditBridgeCommand(self, env)


def _credit_bridge_values(env):
    cmd = env.command_manager.get_term('twist')
    if cmd.bridge_reward_cache_step == env.common_step_counter:
        return cmd.bridge_reward_values
    base = _curriculum_onefoot_values(env)
    robot = env.scene['robot']; data = robot.data
    wheel_ids = getattr(cmd, 'wheel_ids_tensor', cmd.wheel_ids)
    wheels = data.body_link_pos_w[:, wheel_ids]
    ...
    com = data.data.subtree_com[:, data.indexing.root_body_id]
    previous_com = getattr(cmd, '_bridge_previous_com', None)
    previous_support = getattr(cmd, '_bridge_previous_support', None)
    previous_valid = getattr(cmd, '_bridge_previous_valid', None)
    if previous_com is None or previous_valid is None or previous_com.shape[0] != env.num_envs:
        previous_com = com.clone(); previous_support = support.clone()
        previous_valid = torch.zeros(env.num_envs, dtype=torch.bool, device=env.device)
    known = previous_valid[:, None]
    com_v = torch.where(known, (com-previous_com)/env.step_dt, torch.zeros_like(com))
    support_v = torch.where(known, (support-previous_support)/env.step_dt, torch.zeros_like(support))
    cmd._bridge_previous_com = com.clone()
    cmd._bridge_previous_support = support.clone()
    cmd._bridge_previous_valid = torch.ones(env.num_envs, dtype=torch.bool, device=env.device)
    ...
    skate_speed = data.body_link_lin_vel_w[:, wheel_ids[:2], 0].mean(1)
    left_forces = env.scene['onefoot_left'].data.force[..., 2].reshape(env.num_envs, -1).abs()
    right_force = env.scene['onefoot_right'].data.force[..., 2].reshape(env.num_envs, -1).abs().sum(1)
    # Loaded left-only support, matching the environment's own definition, so the
    # balance debt is charged exactly while one skate carries the robot.
    left_only = (env.scene['onefoot_left'].data.found.reshape(env.num_envs, -1).all(1)
                 & (left_forces > .02).all(1)
                 & ~env.scene['onefoot_right'].data.found.reshape(env.num_envs, -1).any(1))
    phi = credit_bridge_potential(base['com_speed'], skate_speed, base['capture_error'],
        fore_aft_capture, left_forces[:, 0], left_forces[:, 1], right_force,
        base['clearance'], base['upright'], base['support_yaw'], onefoot_bad_contact(env))
    action_delta = (env.action_manager.action-env.action_manager.prev_action).square().sum(1)
    collision = env.scene['self_collision'].data.found.reshape(env.num_envs, -1).any(1).float()
    smooth_cost = .10*action_delta+.10*action_over_limit_penalty(env)+.01*joint_torques_l2(env)+collision
    balance_cost = credit_bridge_balance_cost(base['capture_error'], fore_aft_capture, left_only)
    done = env.termination_manager.dones
    physical_failure = torch.zeros_like(done)
    for name in env.termination_manager.active_terms:
        if name != 'time_out':
            physical_failure |= env.termination_manager.get_term(name)
    values = cmd.bridge_reward_state.advance(phi=phi, valid=base['single_support'] > .5,
        com_speed=base['com_speed'], skate_speed=skate_speed, smooth_cost=smooth_cost,
        done=done, physical_failure=physical_failure, balance_cost=balance_cost)
    values['fore_aft_capture_error'] = fore_aft_capture.clone()
    cmd.bridge_reward_values = {name: value.clone() for name, value in values.items()}
    cmd.bridge_reward_cache_step = env.common_step_counter
    return cmd.bridge_reward_values


def credit_bridge_reward(env, component: str):
    """Convert event/step credit to manager rates exactly once (manager applies dt)."""
    return _credit_bridge_values(env)[component]/env.step_dt


def credit_bridge_metric(env, component: str):
    return _credit_bridge_values(env)[component]
